...
import { analyzeTrendCsv, analyzeTrendTicker } from '../lib/api';
...
const TOOL_COPY = {
  en: {
    ...
    noDataBody: 'Run one ticker directly or upload a downloader CSV to start the analysis.',
  ...
  ko: {
    ...
    noDataBody: '티커를 바로 실행하거나, 데스크톱 다운로더 CSV를 업로드해서 분석을 시작하세요.',
...
export function TrendAnalyzerToolContent({ lang, embedded = false }: { lang: SiteLang; embedded?: boolean }) {
  ...
  const [payloadSource, setPayloadSource] = useState<'demo' | 'upload' | 'ticker'>('demo');
  ...
  const sourceLabel =
    payloadSource === 'demo' ? copy.demoSourceLabel : payloadSource === 'ticker' ? copy.tickerSourceLabel : copy.csvSourceLabel;
  const inputLabel =
    payloadSource === 'demo' ? copy.demoSourceLabel : payloadSource === 'ticker' ? tickerInput.trim() || activeTicker : fileLabel;
    ...
    <div className={`trend-tool-shell${embedded ? ' trend-tool-shell--embedded' : ''}`}>
      ...
      <form className="trend-tool-form" onSubmit={handleSubmit}>
        ...
        <div className="trend-tool-form__footer">
          <button type="submit" disabled={busyMode !== null}>
            {busyMode === 'csv' ? copy.processing : copy.submit}
          </button>
